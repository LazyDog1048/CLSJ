...
namespace Editor.Tool
{
    public static class Anim_Editor_Tool
    {
        ...
        {
            List<AnimationClip> clips = Loader_Editor_Tool.GetAllFormSelectFolder_Asset<AnimationClip>();
            // Debug.Log(path);
            ...
            foreach (var clip in clips)
            {
                ...
                EditorCurveBinding binding = EditorCurveBinding.PPtrCurve("", typeof(SpriteRenderer), "m_Image");
        ...
        public static void RemoveFirstBlank(AnimationClip clip)
        {
            EditorCurveBinding binding = EditorCurveBinding.PPtrCurve("Body", typeof(SpriteRenderer), "m_Sprite");
            ObjectReferenceKeyframe[] keyFrames = AnimationUtility.GetObjectReferenceCurve(clip,binding);
            if (keyFrames == null)
        ...
        public static Dictionary<string,AnimationClip> GetStateAnimDic(RuntimeAnimatorController runtimeAnimator)
        {
            Dictionary<string,AnimationClip> dic = new Dictionary<string, AnimationClip>();
            AnimatorOverrideController animatorOverrideController = runtimeAnimator as AnimatorOverrideController;
            if(animatorOverrideController == null)
                return GetStateAnimDic(runtimeAnimator as AnimatorController);
            AnimatorController animatorController = (AnimatorController) animatorOverrideController.runtimeAnimatorController;
            ...
            {
                string stateName = animatorState.state.name;
                Motion motion = animatorState.state.motion;
                if (motion is AnimationClip clip)
                {
                    foreach (var keyValue in overrides)
                    {
                        if (keyValue.Key == clip)
                        {
                            dic.Add(stateName, keyValue.Value != null ? keyValue.Value : keyValue.Key);
                            // Debug.Log($"{stateName}  {keyValue.Value == null}  {keyValue.Key.length}");
                            break;
                        }    
                    }
        ...
        private static Dictionary<string,AnimationClip> GetStateAnimDic(AnimatorController animatorController)
        {
            Dictionary<string,AnimationClip> dic = new Dictionary<string, AnimationClip>();
            ...
            foreach (var animatorState in ams)
            {
                ...
                if (motion is AnimationClip clip)
                {
                    // Debug.Log($"{stateName}  {clip.name} {clip.length}");
                    dic.Add(stateName,clip);
                }
            }
            return dic;
        }
        
        public static Dictionary<int,float> GetHashStateAnimDic(RuntimeAnimatorController runtimeAnimator)
        {
            Dictionary<int,float> dic = new Dictionary<int, float>();
            AnimatorOverrideController animatorOverrideController = runtimeAnimator as AnimatorOverrideController;
            if(animatorOverrideController == null)
                return GetHashStateAnimDic(runtimeAnimator as AnimatorController);
            AnimatorController animatorController = (AnimatorController) animatorOverrideController.runtimeAnimatorController;
            List<KeyValuePair<AnimationClip, AnimationClip>> overrides = new List<KeyValuePair<AnimationClip, AnimationClip>>();
            // 获取所有的动画片段重写映射
            animatorOverrideController.GetOverrides(overrides);
            ...
            foreach (var animatorState in ams)
            {
                int stateNameHash = AnimaHash.ToAnimHash(animatorState.state.name);
                Motion motion = animatorState.state.motion;
                if (motion is AnimationClip clip)
                {
                    foreach (var keyValue in overrides)
                    {
                        if (keyValue.Key == clip)
                        {
                            dic.Add(stateNameHash, keyValue.Value != null ? keyValue.Value.length : keyValue.Key.length);
                            // Debug.Log($"{stateName}  {keyValue.Value == null}  {keyValue.Key.length}");
                            break;
                        }    
                    }
        ...
        private static Dictionary<int,float> GetHashStateAnimDic(AnimatorController animatorController)
        {
            Dictionary<int,float> dic = new Dictionary<int, float>();
            ...
            foreach (var animatorState in ams)
            {
                ...
                {
                    // Debug.Log($"{stateName}  {clip.name} {clip.length}");
                    dic.Add(stateNameHash,clip.length);
                }
            }
            return dic;
        }
        ...
        private static AnimationClip SpriteClip(string texturePath,string savePath = "")
        {
            ...
            EditorCurveBinding binding = EditorCurveBinding.PPtrCurve("Body", typeof(SpriteRenderer), "m_Sprite");
            // EditorCurveBinding binding = EditorCurveBinding.PPtrCurve("", typeof(Image), "m_Sprite");
            // EditorCurveBinding binding = EditorCurveBinding.PPtrCurve("Body", typeof(Image), "m_Sprite");
